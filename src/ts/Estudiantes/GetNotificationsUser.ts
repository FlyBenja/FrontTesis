...
export const getNotificationsUser = async (user_id: number): Promise<{ notification_id: number; message: string }[]> => {
  try {
    ...
    const response = await axios.get(`http://localhost:3000/api/student/notifications/${user_id}`, {
      headers: {
