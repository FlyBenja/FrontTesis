...
import { getNotificationsUser } from '../../ts/Estudiantes/GetNotificationsUser';  // New API for role 1

const DropdownNotification = () => {
  // State to control the visibility of the dropdown
  const [dropdownOpen, setDropdownOpen] = useState(false);
  // State to manage notification alert status
  const [notifying, setNotifying] = useState(true);
  // State to store user role
  const [role, setRole] = useState<number | null>(null);
  // State to store the sede_id from the profile data
  const [sedeId, setSedeId] = useState<number | null>(null);
  // State to store the user_id from the profile data
  const [userId, setUserId] = useState<number | null>(null);
  // State to store notifications data
  const [notifications, setNotifications] = useState<{ notification_id: number; message: string }[]>([]);
  ...
  useEffect(() => {
    ...
    // Fetch user profile data and extract sede_id and user_id
    const fetchProfileData = async () => {
      try {
        const perfilData: PerfilData = await getDatosPerfil();
        setSedeId(perfilData.sede);
        setUserId(perfilData.user_id);  // Assuming this property exists in profile data
      } catch (error) {
  ...
  useEffect(() => {
    // Fetch notifications for role 3 (Administrator) using sedeId
    if (role === 3 && sedeId) {
      const fetchNotifications = async () => {
        try {
          ...
          // Handle errors in fetching admin notifications
        }
      };
    ...
    // Fetch notifications for role 1 (User) using userId
    if (role === 1 && userId) {
      const fetchNotificationsForUser = async () => {
        try {
          ...
          // Handle errors in fetching user notifications
        }
      };
              ...
              {notifications.map((notification) => (
                <li key={notification.notification_id}>
                  <Link className="flex flex-col gap-2.5 border-t border-stroke px-4.5 py-3 hover:bg-gray-2 dark:border-strokedark dark:hover:bg-meta-4" to="#">
                    ...
                    <p className="text-xs">Fecha desconocida</p>
                  </Link>
                </li>
              ))}
            </ul>
