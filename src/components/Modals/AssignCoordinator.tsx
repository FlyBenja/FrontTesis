...
const AssignCoordinatorModal: React.FC<AssignCoordinatorProps> = ({ isOpen, onClose, onAssigned }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const selectedCoordinator = coordinators.find(coord => coord.user_id === userId)
      const selectedSede = sedes.find(sede => sede.sede_id === sedeId)
      
      await assignsHeadquartersCoordinator(userId, sedeId)
      Swal.fire({
        icon: "success",
        title: "¡Coordinador asignado!",
        text: `El coordinador "${selectedCoordinator?.name}" ha sido asignado exitosamente a la sede "${selectedSede?.nameSede}".`,
