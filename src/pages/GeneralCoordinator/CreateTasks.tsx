...
import AyudaCreaTareas from "../../components/Tours/Administrator/TourCreatesTasks"
  ...
  useEffect(() => {
    const fetchTareas = async () => {
      if (selectedCurso && selectedAño) {
        const perfil = await getDatosPerfil() // Fetch user profile
        const tareasRecuperadas = await getTareas(perfil.sede, Number(selectedCurso), Number(selectedAño))
        ...
        {tareas.length > 0 ? (
          <div id="tareas-list" className="space-y-4">
            {currentTareas.map((tarea) => (
              <div key={tarea.task_id} className="p-4 bg-white dark:bg-boxdark rounded-lg shadow-md">
                <div>
