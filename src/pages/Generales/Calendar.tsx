...
const Calendar: React.FC = () => {
  ...
  return (
    <>
      <Breadcrumb pageName="Calendario" /> {/* Breadcrumb for navigation */}
      <div className="bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 flex flex-col items-center justify-center px-2 sm:min-h-screen sm:py-6 py-3 mt-[-60px]">
        <div className="w-full max-w-5xl bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
          {/* Header with navigation buttons for the month */}
          <header className="flex justify-between items-center mb-6">
            <button
              onClick={handlePrevMonth} // Go to previous month
            ...
            <h2 className="text-lg sm:text-xl font-bold mt-3 sm:mt-0">{currentDate.format("MMMM YYYY")}</h2> {/* Display current month and year */}
            <button
              onClick={handleNextMonth} // Go to next month
              className="text-sm sm:text-lg font-bold px-2 py-1 bg-gray-200 dark:bg-gray-700 rounded hover:bg-gray-300 dark:hover:bg-gray-600 transition"
            >
              Next &gt;
            </button>
          ...
          <table className="table-fixed w-full border-collapse border border-gray-200 dark:border-gray-700">
            ...
            <tbody>
              {/* Render each row of the calendar (6 rows total) */}
              {Array.from({ length: 6 }, (_, i) => (
                <tr key={i}>
                  {/* Render each day in the row */}
                  {days.slice(i * 7, (i + 1) * 7).map((day, j) => {
                    const tareasDelDia = day !== null ? getTareasForDate(day) : []; // Get tasks for the day
                    return (
                      <td
                        key={j}
                        role="button" // Make the day clickable
                        tabIndex={0} // Make it focusable
                        className={`h-20 text-center border border-gray-200 dark:border-gray-700 p-2 
                          ${day
                            ? isToday(day) // Highlight the current day
                              ? "bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 font-bold"
                              : tareasDelDia.length > 0 // Highlight the day with tasks
                        ...
                        {tareasDelDia.length > 0 && (
                          <div className="text-xs mt-1">
                            {tareasDelDia.map((tarea, index) => (
                              <div key={index}>{tarea.title}</div> // Display the title of each task
                            ))}
                          </div>
                        )}
                      </td>
